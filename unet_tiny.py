...
class Model(nn.Module):
    ...
    def forward(self, x, audio_feat):
        ...
        # audio_feat = self.audio_encoder(audio_feat.transpose(1, 2))
        audio_feat = audio_feat.view(audio_feat.size(0), audio_feat.size(1), 24*32)
        audio_feat = self.audio_encoder(audio_feat.permute(0, 2, 1))
        audio_feat = audio_feat.permute(0, 2, 1)
        audio_feat = audio_feat.view(audio_feat.size(0), audio_feat.size(1), 32, 32)
        # audio_feat = audio_feat.reshape(audio_feat.size(0), 12, 32, 32)
        # print('audio_feat_model: ', audio_feat.shape)
