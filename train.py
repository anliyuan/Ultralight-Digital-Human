...
from loss_utils import grayscale_l1_loss
import random
...
def get_args():
    ...
    parser.add_argument('--grayscale_loss_weight', type=float, default=0.0)
    ...
    for e in range(epoch):
        ...
        with tqdm(total=len(dataset), desc=f'Epoch {e + 1}/{epoch}', unit='img') as p:
            for batch in train_dataloader:
                ...
                loss_grayscale = grayscale_l1_loss(preds, labels, criterion)
                if use_syncnet:
                    loss = (
                        loss_pixel
                        + loss_PerceptualLoss * 0.01
                        + args.grayscale_loss_weight * loss_grayscale
                        + 10 * sync_loss
                    )
                else:
                    loss = (
