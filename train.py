...
def get_args():
    ...
    parser.add_argument('--syncnet_warmup_epochs', type=int, default=0)
    ...
    for e in range(epoch):
        ...
        with tqdm(total=len(dataset), desc=f'Epoch {e + 1}/{epoch}', unit='img') as p:
            for batch in train_dataloader:
                ...
                if use_syncnet:
                    sync_weight = syncnet_weight_for_epoch(
                        e, base_weight=10.0, warmup_epochs=args.syncnet_warmup_epochs
                    )
                    loss = loss_pixel + loss_PerceptualLoss*0.01 + sync_weight*sync_loss
                else:
