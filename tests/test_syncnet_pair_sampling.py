...
class SyncnetPairSamplingTests(unittest.TestCase):
    ...
    def test_random_negative_pair_creates_mismatch(self):
        rng = random.Random(7)
        audio_idx, label = sample_syncnet_audio_index(
            5, 20, negative_pair_prob=1.0, negative_pair_mode="random", rng=rng
        )
        self.assertNotEqual(audio_idx, 5)
        self.assertEqual(label, 0.0)

    def test_nearby_negative_pair_stays_within_window(self):
        rng = random.Random(7)
        audio_idx, label = sample_syncnet_audio_index(
            50,
            100,
            negative_pair_prob=1.0,
            negative_pair_mode="nearby",
            negative_pair_window=10,
            rng=rng,
        )
        self.assertGreaterEqual(audio_idx, 40)
        self.assertLessEqual(audio_idx, 60)
        self.assertNotEqual(audio_idx, 50)
        self.assertEqual(label, 0.0)
    ...
    def test_invalid_arguments_raise(self):
        ...
        with self.assertRaises(ValueError):
            sample_syncnet_audio_index(0, 10, negative_pair_mode="bad")
        with self.assertRaises(ValueError):
            sample_syncnet_audio_index(0, 10, negative_pair_window=-1)
