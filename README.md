...
# 开源这个项目的时候我从来没想过会得到这么多的关注，所以不论是代码风格还是稳定性都不是很好。在这次修改readme的时候，这个repo已经有1607个star了，感谢各位喜欢这个repo，最近几个月时间我会把这个repo的代码进行一个比较大的修改（集中修改之前issue里提出来的一些问题，以及pr里面的一些建议。也会让代码更易读）。同时也会分批开源python流式推理的代码和c++流式推理的代码，其中c++流式推理的代码可以部署在移动端，加我vx想要购买的同学可以再多等等，我会尽快。（看到很多项目已经集成了这个repo，但都没达到我个人的预期，所以我还是把它开源出来）。敬请期待。
